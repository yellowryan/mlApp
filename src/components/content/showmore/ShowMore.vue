<template>
  <div id="shortcut-container">
    <ul class="shortcut-ul">
      <li class="shortcut-li" v-for="(item, index) in list" :key="index">
        <a href="javascript:void(0)">
          <span
            :style="{background:item.url,backgroundRepeat:'no-repeat',
            backgroundSize:'15px',
            backgroundPosition:'center center'}"
            class="shortcut"
          ></span>
          <span>{{item.text}}</span>
        </a>
      </li>
    </ul>
  </div>
</template>

<script>
export default {
  data() {
    return {
      list: [
        {
          text: "首页",
          url:
            "url(data:image/png;base64,)"
        },
        {
          text: "分类搜索",
          url:
            "url(data:image/png;base64,)"
        },
        {
          text: "我的京东",
          url:
            "url(data:image/png;base64,)"
        },
        {
          text: "浏览记录",
          url:
            "url(data:image/png;base64,)"
        },
        {
          text: "我的关注",
          url:
            "url(data:image/png;base64,)"
        },
        {
          text: "分享",
          url:
            "url(data:image/png;base64,)"
        }
      ]
    };
  }
};
</script>

<style lang="less" scoped>
#shortcut-container {
  height: 0;
}
.shortcut-ul {
  z-index: 99;
  position: absolute;
  top: 48px;
  right: 12px;
  width: 125px;
  border-radius: 4px;
  background: rgba(0, 0, 0, 0.9);
  &::before {
    position: absolute;
    top: -5px;
    right: 5px;
    width: 10.5px;
    height: 5px;
    background: url(data:image/png;base64,)
      no-repeat 50%;
    background-size: 100% 100%;
    content: "";
  }
  .shortcut-li {
    display: block;
    position: relative;
    margin-left: 40px;
    border-bottom: 1px solid hsla(0, 0%, 100%, 0.2);
    height: 40px;
    line-height: 40px;
    z-index: 1000;
    background: transparent;
    a {
      display: block;
      border: 0;
      width: 100%;
      font-size: 14px;
      color: #fff;
      z-index: 1000;
      .shortcut {
        position: absolute;
        top: 0;
        left: -40px;
        width: 40px;
        height: 40px;
      }
    }
  }
}
</style>
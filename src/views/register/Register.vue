<template>
  <div id="login">
    ...
    <div>
      ...
      <p class="input-container">
        <input
          type="password"
          placeholder="再次确认密码"
          class="acc-input password2"
          v-model="pwdValueCheck"
        />
    ...
    <button
      class="btn login-btn"
      @click="registerClick"
      :class="{'btn-active':inputValue&&pwdValue&&pwdValueCheck}"
    >注&nbsp;册</button>
...
export default {
  ...
  methods: {
    clearText() {
      (this.inputValue = ""), (this.$refs.usertip.innerText = "");
    },
    back() {
      this.$router.back();
    },
  ...
  watch: {
    inputValue() {
      if (this.inputValue == "") {
        (this.userDisplay = "none"), (this.$refs.usertip.innerText = "");
      } else {
